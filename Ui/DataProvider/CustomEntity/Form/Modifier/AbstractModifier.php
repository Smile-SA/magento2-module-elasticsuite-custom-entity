...
abstract class AbstractModifier implements ModifierInterface
{
    ...
    const FORM_NAME = 'custom_entity_entity_form';

    const DATA_SOURCE_DEFAULT = 'entity';

    const DATA_SCOPE_PRODUCT = 'data.entity';

    /**
     * Name of default general panel
     */
    const DEFAULT_GENERAL_PANEL = 'general-information';
    ...
    protected function getNextAttributeSortOrder(array $meta, $attributeCodes, $defaultSortOrder, $iteration = 1)
    {
        ...
        foreach ($meta as $groupMeta) {
            $defaultSortOrder = $this->_getNextAttributeSortOrder($groupMeta, $attributeCodes, $defaultSortOrder, $iteration);
        }
        ...
        if (isset($meta['children'])) {
            foreach ($meta['children'] as $attributeCode => $attributeMeta) {
                if ($this->startsWith($attributeCode, self::CONTAINER_PREFIX)) {
                    $defaultSortOrder = $this->_getNextAttributeSortOrder($attributeMeta, $attributeCodes, $defaultSortOrder, $iteration);
    ...
    protected function getFirstPanelCode(array $meta)
    {
        ...
        foreach ($meta as $fieldSetName => $fieldSetMeta) {
            if (isset($fieldSetMeta['arguments']['data']['config']['sortOrder']) && (null === $min || $fieldSetMeta['arguments']['data']['config']['sortOrder'] <= $min)) {
